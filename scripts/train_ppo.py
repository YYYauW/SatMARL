...
def algorithm_metadata(
    ...
    centralized = args.algorithm == "mappo"
    return {
        "name": args.algorithm,
        "family": "policy_gradient_marl",
        "training_mode": (
            "centralized_critic_decentralized_actor"
            if centralized
            else "independent_critic_shared_actor"
        ),
        ...
        "credit_assignment": "gae_individual_reward_plus_team_bonus",
        "policy_samples": "decision_opportunities_only",
        "network": {
...
def ppo_update(
    ...
    decision_rows = torch.count_nonzero(masks, dim=1) > 1
...
def main() -> None:
    ...
    parser.add_argument("--algorithm", choices=["ippo", "mappo"], required=True)
    ...
    _, local_sample, global_sample, masks_sample = flatten_local_all(observations)
    ...
    try:
        for episode in range(start_episode, args.episodes + 1):
            if stop_file.exists():
                ...
                atomic_write_json(metrics_path, metrics)
                save_checkpoint(checkpoint_path, model, optimizer, metrics, args.algorithm)
                break
            ...
            for _ in range(args.max_steps):
                agents, local_obs, global_state, masks = flatten_local_all(observations)
            ...
            update_metrics = ppo_update(model, optimizer, rollout, args, device, rng)
            loss_window.append(update_metrics["loss"])
            summary = env.summary()
